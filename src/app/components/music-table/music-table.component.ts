...
export class MusicTableComponent implements OnInit, OnChanges {
  ...
  toggleSelection(song: SongDisplay, event: Event): void {
    ...
    if (isSelected) {
      ...
      if (this.deleteIdList.length > 5) {
        alert('Select less than or equal to 5 items to delete!');
      }
    } else {
  ...
  /**
   *
   * @param currentColumn Column data of whose sorting function is called
   * @param type The type defines the sort type
   */
